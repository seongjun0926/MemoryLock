...
/*			if (characteristic.getUuid().equals(UUID_HUMIDITY_DATA)) {
			...
			if (service != null) {
				switch (msg.what) {
					...
					case MSG_DEVICE_WRITE_INT:
						if (service.mState == State.CONNECTED && service.mGatt != null) {
							service.writeVal((Integer)msg.obj);
						}
						break;
	...
	public void writeVal(int val) {
		...
		if(mGatt != null)
		{
			...
			Log.i(TAG,"UUID : "+UUID_BLUEINNO_PROFILE_SERVICE_UUID);
			Log.i(TAG,"blueService : "+blueService);

			if (blueService != null) {
				BluetoothGattCharacteristic blueCharacteristic = blueService.getCharacteristic(UUID_BLUEINNO_PROFILE_SEND_UUID);
				Log.i(TAG,"2");
				Log.i(TAG,"UUID 2: "+UUID_BLUEINNO_PROFILE_SEND_UUID);
				Log.i(TAG,"blueService : "+blueCharacteristic);
				...
				if (blueCharacteristic != null) {
					...
					Log.i(TAG,"blueCharacteristic val1 : "+val);
					Log.i(TAG,"blueCharacteristic val2 : "+BluetoothGattCharacteristic.FORMAT_UINT8);
					Log.i(TAG,"blueCharacteristic val3 : "+blueCharacteristic);
